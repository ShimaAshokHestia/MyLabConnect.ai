...
export interface KiduTabbedFormModalProps {
  show: boolean;
  onHide: () => void;
  title: string;
...
const KiduTabbedFormModal: React.FC<KiduTabbedFormModalProps> = ({
  show,
  onHide,
  title,
  headerFields,
  ...
  // ── Header form state ─────────────────────────────────────────────────────
  const buildHeaderInitial = () => {
    const v: Record<string, any> = {};
    headerFields.forEach((f) => {
      v[f.name] = f.type === "select" ? "" : "";
    });
    return v;
  };
        ...
        <div className="ktf-top-fields">
          {headerFields.map((field) => (
            <div
              ...
              style={{ flex: field.colWidth === 3 ? "0 0 25%" : field.colWidth === 4 ? "0 0 33.33%" : field.colWidth === 12 ? "0 0 100%" : "1 1 0" }}
            >
                ...
                <tr>
                  {currentTab.columns.map((col) => (
                    <th key={col.key} className="ktf-th">
                      {col.label}
                      {col.required && <span className="ktf-required"> *</span>}
                    </th>
                  ))}
...
export default KiduTabbedFormModal;
