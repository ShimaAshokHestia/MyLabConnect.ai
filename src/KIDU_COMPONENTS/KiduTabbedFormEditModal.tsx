...
const KiduTabbedFormEditModal: React.FC<KiduTabbedFormEditModalProps> = ({
  ...
  const buildHeaderFromInitial = (): Record<string, any> => {
    ...
    headerFields.forEach((f) => {
      if (f.type === "toggle") {
        v[f.name] = initialHeaderData?.[f.name] ?? f.defaultValue ?? true;
      } else {
        v[f.name] = initialHeaderData?.[f.name] ?? f.defaultValue ?? "";
      }
    });
    // Ensure isActive exists
    if (!v.hasOwnProperty("isActive")) {
      v.isActive = initialHeaderData?.isActive ?? true;
    }
    return v;
  };
  ...
  const buildTabFromInitial = (): Record<string, Record<string, any>[]> => {
    ...
    tabs.forEach((tab) => {
      // If we have initial data for this tab, use it, otherwise create an empty row
      if (initialTabData?.[tab.key] && initialTabData[tab.key].length > 0) {
        initial[tab.key] = initialTabData[tab.key];
      } else {
        ...
        <div className="ktf-modal-wrapper">
          <div className="ktf-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ktf-footer">
              ...
              >
                {isSubmitting ? (
                  <><span className="ktf-spinner" /> Updating...</>
                ) : (
                  submitButtonText
                )}
              </button>
