// src/Pages/AppAdmin/DentalOffice/List.tsx
...
import DentalOfficeAdminCreateModal from "./Create";
import DentalOfficeAdminEditModal from "./Edit";
import DentalOfficeAdminViewModal from "./View";
import DentalOfficeService from "../../../../DSO_ADMIN_CONNECT/Services/Masters/DsoDentalOffice.services";

// ── Column definitions ────────────────────────────────────────────────────────
// dsoName column added for AppAdmin — shows all offices across all DSOs
const columns: KiduColumn[] = [
  ...
  {
    // New column — shows which DSO this office belongs to
    key: "dsoName",
    label: "DSO Master",
    enableSorting: true,
    enableFiltering: true,
    filterType: "text",
    render: (value, row) => <span>{value || `DSO #${row.dsoMasterId}`}</span>,
  },
  {
    key: "isActive",
    label: "Status",
...
// ── Component ─────────────────────────────────────────────────────────────────
const DentalOfficeAdminList: React.FC = () => {
  ...
  return (
    <>
      <KiduServerTableList
        ...
        subtitle="Manage all Dental Office Master data"
        columns={columns}
      ...
      {/* ── Edit / View modals — only mounted when a record is selected ───── */}
      {recordId > 0 && (
        <>
          <DentalOfficeAdminEditModal
            show={showEdit}
...
export default DentalOfficeAdminList;